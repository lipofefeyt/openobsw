/*
 * PUS task — TC dispatch and service handlers S1/S17/S20.
 *
 * This task owns the TC dispatcher and all PUS service contexts. It blocks
 * on the TC frame queue produced by the TMTC task, dispatches each frame
 * through the routing table, and notifies the TMTC task to drain any TM
 * packets generated by the handlers.
 *
 * Priority 3 (below TMTC at 4, above AOCS at 2) means TC processing
 * preempts the control loop but yields to UART I/O.
 */
...
static void pus_task(void *param)
{
    ...
    for (;;) {
        ...
        /* Wake TMTC so it immediately drains any TM we just produced. */
