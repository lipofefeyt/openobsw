 ...
 *
 * Reads raw TC frames from stdin (length-prefixed: 2-byte BE uint16 + frame),
 * feeds them to the dispatcher, and writes TM ack records to stdout.
 *
 * This is the entry point for OpenSVF integration — the OBCEmulatorAdapter
 * will drive this process over a socket or pipe.
 */
#include "obsw/obsw.h"
#include <stdio.h>
#include <stdint.h>
...
/* ------------------------------------------------------------------ */
/* TM responder — writes ack records to stdout                         */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Example S17 are-you-alive handler                                   */
/* ------------------------------------------------------------------ */
...
static obsw_tc_route_t routes[] = {
    {.apid = 0xFFFF, .service = 17, .subservice = 1, .handler = handle_s17_ping, .ctx = NULL},
};
