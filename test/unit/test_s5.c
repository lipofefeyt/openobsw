...
{
    setup_ctx();
    TEST_ASSERT_EQUAL_INT(OBSW_PUS_TM_OK, obsw_s5_report(&s5, OBSW_S5_INFO, 0x0001, NULL, 0));
    uint8_t buf[256];
...
{
    setup_ctx();
    TEST_ASSERT_EQUAL_INT(OBSW_PUS_TM_OK, obsw_s5_report(&s5, OBSW_S5_HIGH, 0x00FF, NULL, 0));
    uint8_t buf[256];
    obsw_sp_packet_t pkt = dequeue_pkt(buf, sizeof(buf));
    TEST_ASSERT_EQUAL_UINT8(5, pkt.payload[1]);
...
{
    TEST_ASSERT_NOT_EQUAL(OBSW_PUS_TM_OK, obsw_s5_report(NULL, OBSW_S5_INFO, 0x0001, NULL, 0));
}
