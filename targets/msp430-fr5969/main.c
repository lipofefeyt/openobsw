...
static void tm_responder(uint8_t flags, const obsw_tc_t *tc, void *ctx)
{
    (void)flags;
    (void)tc;
    (void)ctx;
    /* Dequeue and transmit all pending TM packets */
    uint8_t pkt[OBSW_TM_MAX_PACKET_LEN];
    uint16_t len = 0;
    while (obsw_tm_store_dequeue(&tm_store, pkt, sizeof(pkt), &len) == OBSW_TM_OK) {
        /* Length-prefix then packet */
        uint8_t hdr[2] = {(uint8_t)(len >> 8), (uint8_t)(len & 0xFF)};
        obsw_uart_ops.write(hdr, 2, obsw_uart_ops.ctx);
        obsw_uart_ops.write(pkt, len, obsw_uart_ops.ctx);
    }
}
...
int main(void)
{
    ...
    obsw_tc_dispatcher_init(
        &dispatcher, routes, sizeof(routes) / sizeof(routes[0]), tm_responder, NULL);
    ...
    while (1) {
        ...
        /* 2. Read TC frame if available (length-prefixed) */
        uint8_t hdr[2];
        if (obsw_uart_ops.read(hdr, 2, obsw_uart_ops.ctx) == 2) {
            uint16_t frame_len = (uint16_t)((hdr[0] << 8) | hdr[1]);
            if (frame_len > 0 && frame_len <= sizeof(frame)) {
                /* Blocking read of the full frame */
                uint16_t received = 0;
                while (received < frame_len) {
                    int n = obsw_uart_ops.read(
                        frame + received, frame_len - received, obsw_uart_ops.ctx);
                    if (n > 0)
                        received += (uint16_t)n;
                }
